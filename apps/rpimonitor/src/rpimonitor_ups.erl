...
-export([start_link/0, stop/0, get_voltage/0, get_current/0, get_power/0]).
-export([handle_call/3, handle_cast/2, init/1]).
-behaviour(gen_server).

-compile([
	{nowarn_unused_function, [{read_shunt_voltage_mv, 1}]}
]).
...
-define(INA219_CONFIGURATION,
	<<
		0:2,
		?INA219_VOLTAGE_RANGE_32V:1,
		?INA219_DIV_8_320MV:2,
		?INA219_ADCRES_12BIT_32S:4,
		?INA219_ADCRES_12BIT_32S:4,
...
write_config(I2C) ->
	i2c:write(
		I2C,
		<<?INA219_REG_CONFIG:8, ?INA219_CONFIGURATION/binary>>
	).

write_calibration(I2C) ->
	i2c:write(
		I2C,
		<<?INA219_REG_CALIBRATION:8, ?INA219_DEFAULT_CALVALUE:16>>
	).
